...
public class AdminFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if(servletPath.equals("/co/register")||servletPath.equals("/co/login")) {
			...
			if(session.getAttribute("username")!=null) {
				...
				//根据是否有访问权限，判断
				if(user.get("userType").equals("co")) {
					//类型正确，放行
					chain.doFilter(request, response);
					//System.out.println("servlet path:"+req.getServletPath());
				}else {
					req.setAttribute("httpUrl", "/co/login");
					req.setAttribute("info", "您没有权限访问，即将跳转登陆页面！");
					req.setAttribute("title", "访问失败");
					req.getRequestDispatcher("/co/info.jsp").forward(req, response);
